// RUN: triton-opt %s -split-input-file -triton-amdgpu-insert-instruction-sched-hints -triton-amdgpu-lower-insert-instruction-sched-hints='variant=iglp0' | FileCheck %s -check-prefix=INSERT_IGLP0
// RUN: triton-opt %s -split-input-file -triton-amdgpu-insert-instruction-sched-hints -triton-amdgpu-lower-insert-instruction-sched-hints='variant=iglp1' | FileCheck %s -check-prefix=INSERT_IGLP1
// RUN: triton-opt %s -split-input-file -convert-triton-to-tritongpu='target=hip:gfx942 num-ctas=1 num-warps=4 threads-per-warp=64' -tritongpu-coalesce -tritonamdgpu-accelerate-matmul='arch-generation-name=gfx942 matrix-instruction-size=32 kPack=1' -tritongpu-remove-layout-conversions -tritonamdgpu-stream-pipeline-v2='num_stages=2' -triton-amdgpu-insert-instruction-sched-hints -decompose-unsupported-amd-conversions   -optimize-amd-lds-usage='target-arch=gfx942' -convert-scf-to-cf -convert-index-to-llvm -allocate-shared-memory -convert-triton-amdgpu-to-llvm='arch=gfx942' | FileCheck %s -check-prefix=INSTR_COUNT


module {
  // INSERT_IGLP0-LABEL: @test_dot_op
  // INSERT_IGLP1-LABEL: @test_dot_op
  // INSTR_COUNT-LABEL: @test_dot_op
  ...
  %loop:3 = scf.for %iv = %lb to %ub step %step iter_args(%a_ptr = %a_ptr_init, %b_ptr = %b_ptr_init, %prev_c = %c_init) -> (tensor<128x32x!tt.ptr<f16>>, tensor<32x128x!tt.ptr<f16>>, tensor<128x128xf32>) {
    ...
    // INSTR_COUNT: amdgpu.instruction_sched_hint
    // INSTR_COUNT-SAME: isBufferLoadsAEnabled = false
    // INSTR_COUNT-SAME: isBufferLoadsBEnabled = false
    // INSTR_COUNT-SAME: numDsReadsA = #amdgpu.InstCounter<8, vector<4xf16>>
    // INSTR_COUNT-SAME: numDsReadsB = #amdgpu.InstCounter<32, vector<1xf16>>
    // INSTR_COUNT-SAME: numDsWritesA = #amdgpu.InstCounter<4, vector<4xf16>>
    // INSTR_COUNT-SAME: numDsWritesB = #amdgpu.InstCounter<4, vector<4xf16>>
    // INSTR_COUNT-SAME: numGlobalLoadsA = #amdgpu.InstCounter<4, vector<4xf16>>
    // INSTR_COUNT-SAME: numGlobalLoadsB = #amdgpu.InstCounter<4, vector<4xf16>>
    // INSTR_COUNT-SAME: numMMAs = #amdgpu.InstCounter<16, tensor<32x32x8xf16>>
